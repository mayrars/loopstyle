            ...
            <ul className="space-y-2 font-medium">
              {data && data.map(category => {
                return category.name !== "New Category" ? 
                  <li key={`Sidenavbar-${category.id}`}>
                      <Link to={`/products/${category.id}`} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
